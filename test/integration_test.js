...
describe('integration', function() {
    ...
    function testFunctionality() {
        ...
        it('should return proper results for a normal query', async () => {
            const result = await request(`http://localhost:${port}/search?q=${encodeURIComponent('"aggregation report" use cases')}`)
            assert.strictEqual(result.response.statusCode, 200)
            assert.strictEqual(result.response.headers['content-type'], 'application/json')
        ...
        it('should properly handle searchProperty', async () => {
            ...
            assert.strictEqual(result.response.statusCode, 200)
            assert.strictEqual(result.response.headers['content-type'], 'application/json')
            assert.strictEqual(result.json.results.length, 19)
            ...
            assert.strictEqual(result.response.statusCode, 200)
            assert.strictEqual(result.response.headers['content-type'], 'application/json')
            assert.strictEqual(result.json.results.length, 13)
        })
